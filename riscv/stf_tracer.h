...
// Copyright (C) 2025, Adam Caughron, Condor Computing
//
...
struct StfTracer
{
                  ...
                  reg_t pc, reg_t npc, trap_t t, std::string debug="")
  {
    ...
    if ((ppn_match && (t.cause() == CAUSE_USER_ECALL || t.cause() == CAUSE_SUPERVISOR_ECALL)) || trace_this_event) {
...
public:
  ...
  bool _trace_complete{false};  // FIXME
...
public:
  ...
  StfTracer(
    ...
    exit_on_stop_opc(exit_on_stop_opc),
    stf_verbose(stf_verbose),
    force_zero_sha(force_zero_sha),
    include_trace_macros(include_trace_macros),
    macro_tracing(macro_tracing),
    insn_num_tracing(insn_num_tracing),
